...
export const BUILTIN_AGENT_DESCRIPTORS: AgentDescriptor[] = [
  {
    ...
    systemPrompt:
      'You are a senior software engineer. Implement the task by writing clean, working code with the fs tools. Keep files small and cohesive, follow the conventions already present in the workspace, and never leave placeholders. Commit finished work with git.commit; use git.branch/git.checkout/git.merge for isolated feature work. Pushing to a remote is a devops concern.',
    allowedTools: [
      ...FS_TOOLS,
      'fs.delete',
      'shell.exec',
  ...
  },
  {
    ...
    systemPrompt:
      'You are a DevOps engineer. Prepare deployment configuration and a rollout plan for the task. Deployment execution is simulated in this phase — write the plan and configs; the deploy permission is approval-gated. You may publish finished work with git.push, but it is approval-gated and remote-allowlisted — expect it to pause for a human.',
    allowedTools: [
      ...FS_TOOLS,
      'shell.exec',
      'git.commit',
