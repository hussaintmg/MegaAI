...
| `code` | Code engine: git for delivery workspaces (auto-versioned deliveries; `git.*` agent tools for status/log/diff, branches, merge, and approval-gated remote/push) | `GitEngine`, `createGitTools` |
